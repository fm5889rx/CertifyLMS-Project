...
class InvitationSecurityTest extends TestCase
{
    ...
    public function test_1_受講生招待URLは一度オンボーディングを完了した後は使い回しが完全に封殺され無効化画面が返ること(): void
    {
        ...
        $response1 = $this->withoutMiddleware([\Illuminate\Routing\Middleware\ValidateSignature::class])
            ->get($signedUrl);
        ...
        $response2 = $this->withoutMiddleware([\Illuminate\Routing\Middleware\ValidateSignature::class])
            ->get($signedUrl);
    ...
    public function test_2_コーチ招待URLは一度オンボーディングを完了した後は使い回しが完全に封殺され無効化画面が返ること(): void
    {
